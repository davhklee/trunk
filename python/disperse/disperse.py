...
for r, subfolder, files in os.walk("merged"):
    if not any([x in r for x in test_list]):
        for f in files:
            count += 1
            src = "/".join([r, f])
            dst = src.replace("merged", "davhklee")
            print(dst)
            os.makedirs(os.path.dirname(dst), exist_ok=True)
            shutil.copy(src, dst)
            os.system("cd davhklee;git add --all")
            dat = random_date("1/1/2007 9:00 AM", "5/25/2024 5:00 PM")
            print(dat)
            os.system("cd davhklee;git commit --date='" + dat + "' -m '" + msg_list[math.floor(random.random() * len(msg_list))] + "'")
